...
export default function DatasetPage({params}:{params:{id:string}}){
 ...
 {plan&&<section className={styles.plan}><div className={styles.panelHead}><div><small>RIVU AI / CLEANING STUDIO</small><h2>Recommended refinement plan</h2></div><Sparkles size={19}/></div><p className={styles.summary}>{plan.summary||plan.dataset_summary}</p><div className={styles.ops}>{plan.operations?.map((o:any,i:number)=><div key={i}><CheckCircle2 size={16}/><b>{o.type.replaceAll("_"," ")}</b><span>{o.column||"dataset"} · {Math.round(o.confidence*100)}% confidence</span><p>{o.reason}</p></div>)}</div><div className={styles.actions}><button onClick={previewPlan} disabled={busy}>Preview changes</button><button className={styles.ai} onClick={executePlan} disabled={running}>{running?"Executing…":"Approve & refine"}</button></div>{preview&&<div className={styles.issues}><strong>Before / after preview</strong>{preview.previews?.map((p:any,i:number)=><p key={i}><span>{p.column||p.op}</span>{(p.before||[]).slice(0,3).join(", ")} → {(p.after||[]).slice(0,3).join(", ")}</p>)}</div>}</section>}</section></>}
 </main>
}
