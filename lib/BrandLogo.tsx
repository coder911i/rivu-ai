...
export default function BrandLogo({href="/", className=""}:{href?:string;className?:string}) {
  const mark: CSSProperties = {width:31,height:31,display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:3,position:"relative",flexShrink:0};
  ...
  return (
    <Link href={href} aria-label="Rivu home" className={className}
      ...
      <span>rivu<span style={{fontWeight:400,color:"#a855f7"}}>ai</span></span>
    </Link>
  );
}
